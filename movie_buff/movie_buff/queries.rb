...
def vanity_projects
  ...
  Movie.joins(:actors).where("movies.director_id = actors.id AND castings.ord = 1").select("movies.id, movies.title, actors.name")
...
def starring(whazzername)
  ...
  letters = whazzername.gsub(" ", "").split('').reduce('%') { |str, char| str += (char + '%') }
  # p letters
  Movie.joins(:actors).where("LOWER(actors.name) LIKE ?", letters)
...
def bad_years
  ...
  #Movie.where("movies.score < 8").select("DISTINCT movies.yr")
end
